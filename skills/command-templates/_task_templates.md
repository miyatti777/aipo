...
| **3. implement** | 実際にワークスペースに実装 | ファイル/ビュー/自動化 | ✅ 確認 |
...
成果物を作成する際、以下を明確に区別する：

| 要件 | 正しいエンティティ | 間違いの例 |
| --- | --- | --- |
| 「Kanbanビューを作成」 | **YAMLデータ** + Boardビュー | ファイルに説明文 |
| 「Timelineビューを作成」 | **YAMLデータ** + Timelineビュー | ファイルに説明文 |
| 「進捗管理DBを作成」 | **YAMLファイル** + 適切なプロパティ | ファイルにテーブル |
| 「テンプレートを作成」 | **Markdownファイル**（テンプレート形式） | 説明だけのファイル |
| 「ガイドを作成」 | **Markdownファイル**（本文あり） | OK |
