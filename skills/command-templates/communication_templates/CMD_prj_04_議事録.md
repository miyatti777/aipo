# CMD_prj_04_議事録

最終更新日時: 2025年12月25日 9:06

# 04 実行: 議事録

## 最初に質問（実行前に回答してください）

- 会議名・目的（必須）
- 開催日時・場所（必須）
- 参加者（必須）
- 議題・アジェンダ（必須）
- 決定事項・アクションアイテム（必須）

## 目的

Rules（`basic/04_pmbok_executing.mdc`）に準拠し、プロジェクト関連会議の議事録を作成します。会議の内容を正確に記録し、決定事項とアクションアイテムを明確化します。

## 必要入力

- 会議名: 会議の名称・目的
- 開催情報: 日時・場所・形式（対面/オンライン）
- 参加者: 出席者・欠席者・役割
- 議題: 討議内容・アジェンダ
- 決定事項: 会議で決まったこと
- アクションアイテム: 誰が・何を・いつまでに

## 実行手順（Rules Steps）

```yaml
- trigger: "議事録|ミーティングノート"
  steps:
    - name: "start_info_collection"
      action: "call basic/pmbok_executing.mdc => meeting_minutes_questions"
      message: "会議議事録を作成します。会議の基本情報を入力してください。"
    - name: "wait_user_responses"
      action: "wait_for_all_answers"
    - name: "confirm_document_creation"
      action: "confirm"
      message: "議事録のドラフトを作成してよいですか？"
    - name: "create_draft"
      action: "create_markdown_file"
      path: "{{patterns.draft_minutes}}"
      template_reference: "basic/pmbok_executing.mdc => meeting_minutes_template"
      message: "{{patterns.draft_minutes}} が生成されました。"
```

## 議事録の構成要素

### 📋 基本情報

- *会議名: 会議の正式名称*
- *目的: 会議の目的・ゴール*
- *日時: 開催日時・所要時間*
- *場所: 開催場所・形式*
- *参加者: 出席者・欠席者・役割*

### 📝 会議内容

- *議題: 討議したトピック・アジェンダ*
- *討議内容: 主要な発言・意見・議論*
- *資料: 使用した資料・参考文書*
- *質疑応答: 質問と回答の内容*

### ✅ 決定事項・アクション

- *決定事項: 会議で決まったこと・合意事項*
- *アクションアイテム: 具体的な行動項目*
- *担当者: 各アクションの責任者*
- *期限: 完了予定日・マイルストーン*
- *次回会議: 次回の予定・議題*

## 生成物

- Flow/.../draft_meeting_minutes.md（ドラフト）
- Stock/.../4_executing/meetings/meeting_minutes_[日付].md（確定版）

## 議事録のベストプラクティス

### ✅ 良い議事録

- *客観的: 事実を客観的に記録*
- *具体的: 曖昧さのない具体的な記述*
- *行動指向: アクションアイテムが明確*
- *追跡可能: 後で確認・フォローできる*

### ❌ 避けるべき議事録

- *主観的: 個人の感想・解釈が混在*
- *曖昧: 「検討する」「考慮する」等の曖昧な表現*
- *行動不明: 誰が何をするか不明確*
- *期限なし: いつまでにやるか不明*

## 議事録の活用方法

### 📊 プロジェクト管理

- *進捗確認: 前回からの進捗状況*
- *課題管理: 課題・リスクの追跡*
- *意思決定: 決定事項の記録・共有*
- *責任明確: 役割・責任の明確化*

### 🔄 コミュニケーション

- *情報共有: 欠席者への情報伝達*
- *認識合わせ: 参加者間の認識統一*
- *エスカレーション: 上位層への報告*
- *外部連携: 関係者との情報共有*

### 📚 ナレッジ管理

- *履歴管理: 意思決定の経緯記録*
- *教訓抽出: プロジェクト完了時の振り返り*
- *ベストプラクティス: 成功事例の蓄積*
- *リスク予防: 過去の課題からの学習*

## アクションアイテム管理

### 📋 5W1H形式

- *Who（誰が）: 担当者・責任者*
- *What（何を）: 具体的な作業内容*
- *When（いつまでに）: 完了期限*
- *Where（どこで）: 実施場所・環境*
- *Why（なぜ）: 目的・背景*
- *How（どのように）: 実施方法・手段*

### 🎯 優先度管理

- *緊急・重要: すぐに対応が必要*
- *重要・非緊急: 計画的に対応*
- *緊急・非重要: 効率的に対応*
- *非緊急・非重要: 必要に応じて対応*

## 次に実行

- 「07_日次タスク作成」でアクションアイテムを日次タスクに展開
- 「05_変更要求」で会議で決まった変更を管理
- 次回会議前に前回議事録の確認・フォローアップ

## 参照Rule

- `.cursor/rules/basic/04_pmbok_executing.mdc`（議事録）
- `.cursor/rules/basic/00_master_rules.mdc`

## トラブルシュート

- 会議中にリアルタイムで記録することを推奨
- 不明な点は会議中に確認・明確化
- 議事録は24時間以内に参加者に共有