...
1. **自律検索（ワークスペース→Slack→Google Drive）**を先に実行して候補を集める
...
- Flow直近（日付フォルダ）にある「タスク/要対応/返事/提出/締切」系のファイルを探す
- 直近の「daily_tasks.md」があれば、そこから"未完了"を拾う
