...
**目的**: 任意のMarkdownファイルからアクションアイテム・タスクを抽出し、今日のタスクリストに追加
...
- **対象ファイル** (必須): タスクを抽出したいMarkdownファイルのパス、または現在開いているファイルを自動認識
...
1. **🔴 緊急・重要セクション**にタスクがある場合：
    ...
    - [ ] **kanさんに確認 - 経営観点での目標を共有いただく**
      - 元: kanさん依頼対応_PF計画FDE統合版.md > アクションプラン > 今日
      - 依頼方法: Slackで簡潔に依頼...
    ```
...
- [ ] **kanさんに確認 - 経営観点での目標を共有いただく**
  - 元: kanさん依頼対応_PF計画FDE統合版.md > アクションプラン
  - 依頼方法: Slackで簡潔に依頼...
...
1. **対象ファイルが指定されていない場合**
    - エラーメッセージ: 「対象ファイルを指定してください。現在のファイルを使用する場合は「このファイル」と入力してください。」
2. **タスクが見つからない場合**
    - 警告メッセージ: 「タスクが見つかりませんでした。ファイルにチェックボックスやアクションアイテムがあるか確認してください。」
3. **自分のタスクが0件の場合**
    - メッセージ: 「あなたに割り当てられたタスクは見つかりませんでした。他のメンバーのタスクを確認しますか？」
4. **今日のタスクファイルが存在しない場合**
...
対象ファイル: kanさん依頼対応_PF計画FDE統合版.md
