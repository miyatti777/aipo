...
└── 202512/
    └── 2025-12-09/
        ├── (既存ファイル)
        └── daily_tasks.md  ← 今日のタスク（Markdownテーブル）
```
...
- **見積時間** (text): 所要時間の見積もり
- **抽出元** (select): ワークスペース / Slack / Google Docs / ユーザー追加
...
| --- | --- | --- | --- | --- | --- |
| Sprint Planningの準備 | 🔴緊急・重要 | ☐ | 1h | ワークスペース | Sprint |
| 1on1の資料作成 | 🔴緊急・重要 | ☐ | 30m | Slack | 1on1 |
| Palmaドキュメント更新 | 🟡重要・非緊急 | ☐ | 2h | ワークスペース | Palma |
| コマンド整備 | 🟡重要・非緊急 | ☐ | 1h | ワークスペース | 内部整備 |
| 週次レビューの準備 | 🟢その他 | ☐ | 30m | Google Docs | レビュー |

**統計情報（テーブル下に追記）:**

```
抽出元: ワークスペース 3件 / Slack 1件 / Google Docs 1件 / ユーザー追加 0件
...
### ステップ4: Flowの日付フォルダにタスクファイル作成 (15秒)

1. **今日の日付フォルダの確認**
    - Flow/YYYYMM/YYYY-MM-DD/ を確認
    - 存在しない場合は自動作成（月フォルダも含む）
2. **タスクファイルを作成または更新**
    - ファイル名: daily_tasks.md
    - 既存ファイルがあれば末尾に追記
    - セクション見出し: ## 📋 今日のタスク
3. **Markdownテーブル形式で作成**
    - タスク名
    - 優先度: 「🔴緊急・重要」「🟡重要・非緊急」「🟢その他」
    - 完了: チェックボックス `- [ ]` 形式
    - 見積時間
    - 抽出元: 「ワークスペース」「Slack」「Google Docs」「ユーザー追加」
    - 関連プロジェクト
4. **タスクデータを追加**
    - 優先度順にソートして記載
5. **抽出元の統計情報を追加**
...
3. **Slack/Google Documentsにアクセスできない場合**
    - 警告メッセージ: 「SlackまたはGoogle Documentsにアクセスできませんでした。ワークスペース内の情報のみでタスクを作成します。」
4. **ユーザーがヒアリングに答えない場合**
...
```
朝9:00にコマンド実行
→ ワークスペース/Slack/Google Docsからタスクを自動収集
→ AIが「他にありますか？」と確認
→ 優先度順に整理されたタスクリストが完成
→ 1日の始まりがスムーズ
```
